...
from libs.util import cmap,SqueezedNorm,pool2d,loadSiteImage
import os
import pdb
...
class InpaintingModel(tf.keras.Model):
    ...
    def train_step(self, data):# data=(masked_imgs, mask, gt_imgs)
        ...
        # 勾配の取得　勾配計産GradientTape()を使うときはfor文使うのは注意！の中でfor文使うとおかしいことになる
        ...
        gradients = tape.gradient(loss, trainable_vars) #勾配計算
        self.optimizer.apply_gradients(zip(gradients, trainable_vars)) #勾配更新
...
#========================================
# PConv + 学習可能な位置特性
## 位置特性を学習によって取得する カーネルに入れない乗算と加算1
## L1ノルム最小化でスパースにする
...
class PKConvLearnSite(InpaintingModel):
    def __init__(self, opeType='mul', obsOnlyL1 = False, **kwargs):
        ...
        self.phase = 0
        #初期値に地下構造マップを挿入
        #StrataPath = f"data{os.sep}nankaiUnderGroundData{os.sep}monokuro_layer3.png"
        StrataImage = np.array(Image.open(f"data{os.sep}nankaiUnderGroundData{os.sep}monokuro_layer3.png"))
        ...
        if opeType=="add":
            ...
            # self.initValue = np.ones([1,self.img_rows,self.img_cols,1])
            self.initValue = StrataImage[np.newaxis, :, :, np.newaxis]
            #self.initValue.shape = (1, 512, 512, 1)
            #pdb.set_trace()
            self.scenter = 1
   ...
   # existmask=False付け足した
    def plotSiteFeature(self, epoch=None, plotSitePath="", existmask=False):
        plt.clf()
        plt.close()
    ...
    def build_pconv_unet(self, masked, mask, training=True):
        ...
        def applySite(x,i):
            ...
            if (i in self.siteLayers):
                ...
                #学習した位置特性マップがsconvoutだから、これに足したりかけたり
